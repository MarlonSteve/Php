...
function esPalindromo($cade) {//preg_replace para eliminar espacios y signos de puntuación, y convierte en minúsculas.
    $cade = preg_replace("/[^a-zA-Z0-9]/", "", strtolower($cade)); //strtolower  se utiliza para convertir una cadena de texto a minúsculas.
    return $cade === strrev($cade);//strrev para determinar si es un palíndromo.
}

if (isset($_POST['cade'])) {
    $cade = $_POST['cade'];
    $resultado = esPalindromo($cade) ? "True" : "False";
} else {
    $cade = '';
    $resultado = '';
}
